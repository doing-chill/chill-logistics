...
public class KafkaConsumerConfig {
    ...
    @Bean
    public ConsumerFactory<String, StockDecreaseCompletedV1> productConsumerFactory() {

        // JSON → StockDecreaseV1 역직렬화를 위한 Deserializer
        JsonDeserializer<StockDecreaseCompletedV1> deserializer = new JsonDeserializer<>(StockDecreaseCompletedV1.class, false);

        // Kafka 메시지 역직렬화 시 허용할 패키지를 명시적으로 지정
        deserializer.addTrustedPackages("chill_logistics.order_server.infrastructure.kafka.dto");
        ...
        properties.put(ConsumerConfig.GROUP_ID_CONFIG, "order-server-group");
    ...
    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, StockDecreaseCompletedV1> productKafkaListenerContainerFactory() {

        ConcurrentKafkaListenerContainerFactory<String, StockDecreaseCompletedV1> factory = new ConcurrentKafkaListenerContainerFactory<>();
